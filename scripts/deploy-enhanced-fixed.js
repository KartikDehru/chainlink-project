...
async function main() {
  ...
  const EnhancedPricePredictionDApp = await hre.ethers.getContractFactory("EnhancedPricePredictionDApp");
  ...
  try {
    ...
    console.log("Current ETH/USD Price:", hre.ethers.formatUnits(ethPrice, 8));
    
    // Get BTC/USD price
    const btcPrice = await dapp.getLatestPrice(1);
    console.log("Current BTC/USD Price:", hre.ethers.formatUnits(btcPrice, 8));
